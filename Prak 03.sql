...
SELECT M_ID, M_Nama, M_Harga, M_Deskripsi
FROM Menu
WHERE M_Harga <= 15000
  AND (LOWER(M_Nama) NOT LIKE '%ayam%' AND LOWER(M_Deskripsi) NOT LIKE '%ayam%');

--2

SELECT
    C.C_Nama AS Customer_Name,
    GROUP_CONCAT(M.M_Nama) AS Purchased_Menus,
    SUM(M.M_Harga) AS Total_Spent
FROM
    Customer C
JOIN
    Transaksi T ON C.C_ID = T.Customer_C_ID
JOIN
    Menu_Transaksi MT ON T.T_ID = MT.Transaksi_T_ID
JOIN
    Menu M ON MT.Menu_M_ID = M.M_ID
WHERE
    C.C_Nama = 'Lisa Davis'
GROUP BY
    C.C_ID;
...
SELECT
    ...
    COUNT(MT.Menu_M_ID) AS Number_of_Menus,
    SUM(M.M_Harga) AS Total_Price
FROM
    Transaksi T
JOIN
    Menu_Transaksi MT ON T.T_ID = MT.Transaksi_T_ID
...
GROUP BY
    T.T_ID
ORDER BY
    Number_of_Menus DESC, Total_Price DESC;
...
FROM
    driver
WHERE
    D_Rating > (SELECT AVG(D_Rating) FROM driver);

-- 5

SELECT
    DATE(T_WaktuPesanan) AS Order_Date,
...
SELECT
    P_Jabatan AS Employee_Type,
    COUNT(P_ID) AS Number_of_Employees,
    AVG(P_Gaji) AS Average_Salary
FROM
    pegawai
GROUP BY
    Employee_Type;
...
SELECT
    P.P_ID AS Employee_ID,
    P.P_Nama AS Employee_Name,
    COUNT(DISTINCT C.C_ID) AS Number_of_Customers
FROM
    pegawai P
JOIN
    Transaksi T ON P.P_ID = T.Pegawai_P_ID
JOIN
    Customer C ON T.Customer_C_ID = C.C_ID
GROUP BY
...
-- -- Calculate Total Income for November
-- SELECT
--     SUM(CASE
--         WHEN T_MetodePembayaran IN ('Debit Card', 'Credit Card') THEN T_Total_Payment * 0.95
--         WHEN T_MetodePembayaran = 'Cash' THEN T_Total_Payment
--         ELSE 0
--     END) AS Total_Income
-- FROM
--     Transaksi
-- WHERE
--     MONTH(T_WaktuPesanan) = 11; -- November

-- -- Calculate Ms. Nuri's Profit
-- SELECT
--     Total_Income * 0.4 AS Ms_Nuri_Profit
-- FROM
--     (
--         -- Subquery to calculate Total Income
--         SELECT
--             SUM(CASE
--                 WHEN T_MetodePembayaran IN ('Debit Card', 'Credit Card') THEN T_TotalPayment * 0.95
--                 WHEN T_MetodePembayaran = 'Cash' THEN T_TotalPayment
--                 ELSE 0
--             END) AS Total_Income
--         FROM
--             Transaksi
...
SELECT
    SUM(T_Total_Payment) AS Total_Income
FROM
    Transaksi
WHERE
    MONTH(T_Waktu_Pesanan) = 11; -- November


SELECT
    SUM(
        CASE
            WHEN T_Metode_Pembayaran IN ('Debit Card', 'Credit Card') THEN T_Total_Payment * 0.95
            WHEN T_Metode_Pembayaran = 'Cash' THEN T_Total_Payment
            ELSE 0
        END
    ) AS Total_Income
FROM
...
-- Calculate the number of transactions and total income for weekdays
SELECT
    'Weekdays' AS Day_Type,
    COUNT(T_ID) AS Number_of_Transactions,
    SUM(T_Total_Payment) AS Total_Income
FROM
    Transaksi
WHERE
    DAYOFWEEK(T_Waktu_Pesanan) BETWEEN 2 AND 6 -- Monday to Friday;

UNION

-- Calculate the number of transactions and total income for weekends
SELECT
    'Weekends' AS Day_Type,
    COUNT(T_ID) AS Number_of_Transactions,
    SUM(T_Total_Payment) AS Total_Income
FROM
    Transaksi
WHERE
    DAYOFWEEK(T_Waktu_Pesanan) IN (1, 7); -- Sunday and Saturday
...
SELECT
    D.D_ID AS Driver_ID,
    D.D_Nama AS Driver_Name,
    COUNT(T.Driver_D_ID) AS Total_Delivered_Menus
FROM
    driver D
LEFT JOIN
    Transaksi T ON D.D_ID = T.Driver_D_ID
GROUP BY
    Driver_ID, Driver_Name
ORDER BY
    Total_Delivered_Menus DESC;
